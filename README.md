...
/qqhub 面板     兼容入口，同上
/诊断           群里跑一次自检，结果压成一条消息
/图床测试       发一张内嵌图片的卡片，验证图床全链路
```

`/qqhub` 直接发送当前群面板；`/qqhub 面板` 保留为兼容入口。成功时只发送一张面板；失败时只回复一条错误。

`/诊断`（别名 `/qqhub 诊断`、`/hub诊断`）把模块、接口、Action、交互桥、图床、
存量、身份七项压进**一条**消息。此前诊断只存在于 WebUI 抽屉里，而需要它的时刻
人正在群里；分多条发会吃掉 QQ 官方本就紧张的回复配额，所以只发一条。

`/图床测试` 是唯一覆盖全链路的检查：发布 → 隧道 → 腾讯抓取 → 客户端渲染。
前面每一环都可能成功而图片仍然裂开，中途没有任何报错。看到棋盘格即全通。
自检图**每次运行都换配色**——图案不变说明你看到的是客户端缓存，不是新抓的图。
...
按钮权限选 `group_manager` 时，Hub 会在**服务端**核对点击者的群内角色
（`author.member_role`，取自 `admin` / `owner`）。

在此之前这一档**没有任何服务端校验**：代码假设 QQ 会在投递回调前拦掉非管理员。
但 `permission.type=1` 管的是按钮**怎么渲染**，官方契约从没承诺压制回调——
也就是说编辑器里看起来最严格的那一档，很可能一直是装饰性的。

角色只能从**群消息**里学到（点击事件不带任何身份信息），因此：

- 没在本群说过话的人**点不了**管理按钮 —— 说一句话即可恢复；
- **群主也算管理**（QQ 把群主报成 `owner` 而不是 `admin`）；
- **Hub 操作员和 AstrBot 管理员无条件放行**，避免刚重启的群没人能点；
- `/诊断` 会显示已知管理员人数，用来区分「校验坏了」和「还没人说过话」。
...
域名被当成**租约**而不是事实：每次发图前、以及后台每分钟，都会跟本机 cloudflared
核对一次。这一步不能省——隧道重启后，发布照样成功、卡片照样发出，**唯一的症状是
所有人看到的图都裂了，没有任何报错**。这是整条链路上唯一完全静默的故障。

* 域名变了 → 自动换新，并作废旧 slot（否则「同样的图复用旧 URL」会指向死域名）；
* cloudflared 没响应 → **沿用**上次的域名（它可能还活着，丢掉则必然失败）；
* `image_host_base_url` **填了值** → 一律不动。那是你对自己基础设施的承诺
  （具名隧道 / 反代），自动发现无权推翻，否则固定域名方案就废了。
...
才启动它，于是形成一个死结：拿不到地址 → 不启动 → 没有守护 → **永远不再重试**。
cloudflared 后来起来了也没用，必须人工重载插件——而日志只会一直说
「拿不到公网地址」，看不出它其实已经放弃了。

现在每分钟重试一次，隧道一出现就自动上线并打印
`[QQHub] 图床已自动恢复：https://...`。cloudflared 和 AstrBot 是各自独立启动的，
「开机时隧道还没好」是常态而不是故障。
...
这个计数是链路可用的唯一硬证据：发布是纯本地动作，无论公网能否访问都会成功返回
URL，只有真的有人来取过字节，才说明腾讯够得着这个地址。
