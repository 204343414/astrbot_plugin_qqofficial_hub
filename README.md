...
| 页面 | 用途 |
| --- | --- |
| **卡片编辑器**（`pages/panels`） | 编辑 Markdown、按钮、一次性与归属开关，并试发到群 |
| **运行诊断**（`pages/zz-diagnostics`） | **只读**，排查「插件有没有接上」 |

> 页面按目录名字母序排列，编辑器排在首位，打开插件即进入可编辑页面。

### 运行诊断页

WebUI 插件页新增 **运行诊断**（`pages/diagnostics`），实时枚举：
