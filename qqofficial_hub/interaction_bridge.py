...
def install(owner: str, callback: Callback) -> int:
    ...
    state["original_interaction"] = getattr(module.botClient, "on_interaction_create", None)
...
async def _dispatch(client: Any, interaction: Any) -> None:
    ...
    if lock is None:
        ...
        state["lock"] = lock
    async with lock:
        if interaction_id in state["seen"] or interaction_id in state["inflight"]:
            logger.warning("[QQHub] Duplicate interaction ignored: %s", interaction_id)
            return
        state["inflight"].add(interaction_id)
        state["seen"][interaction_id] = time.monotonic()
